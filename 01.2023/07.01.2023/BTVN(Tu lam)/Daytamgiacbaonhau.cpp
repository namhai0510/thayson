...
bool Dnt (int it1, int it2, tamgiac D) {
    long long x1=abs(D.a1-D.b1),y1=abs(D.a2-D.b2),x2=abs(D.a1-D.c1),y2=abs(D.a2-D.c2);
    ...
    if (S-(S1+S2+S3)<=espilon) return true;
    return false;
}
void Nhap () {
    cin >> n;
    for (int i=0;i<n;i++) {
        ...
        long long x1=abs(pos[i].a1-pos[i].b1),y1=abs(pos[i].a2-pos[i].b2),x2=abs(pos[i].a1-pos[i].c1),y2=abs(pos[i].a2-pos[i].c2);
        double S=abs(x1*y2-x2*y1)/2;
        pos[i].dientich=S;
    }
    sort (pos,pos+n,xet);
...
void QuyHoachDong () {
    ...
    for (int i=1; i<n; i++){
        F[i] = 1;
        for (int j=0; j<=i-1; j++){
            if (pos[j].dientich<pos[i].dientich && Dnt(pos[j].a1,pos[j].a2,pos[i])==true && Dnt(pos[j].b1,pos[j].b2,pos[i])==true && Dnt(pos[j].c1,pos[j].c2,pos[i])==true){
                ///f[i]=max(f[j]+1, f[i]);
                if (F[i] < F[j] + 1) {
                    F[i] = F[j] + 1;
                }
